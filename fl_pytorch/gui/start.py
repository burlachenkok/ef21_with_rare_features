    ...
    for fileName in fileNames:
        ...
        for job_id in list_job_ids:
            ...
            if job_id in simulations_stats:
                ...
                if H['args'].algorithm.lower() == "fedavg":
                    H["comment"] = "GD"
                    H['args'].comment = "GD"
                else:
                    H["comment"] = "EF21 (BASE)"
                    H['args'].comment = "EF21 (BASE)"

                    for option in H['args'].algorithm_options.split(","):
                        kv = option.split(":")
                        if kv[0] == "ef21_scaled":
                            H["comment"] = "EF21 (OUR)"
                            H['args'].comment = "EF21 (OUR)"
...
def toAxisName(axis):
    ...
    if axis == "# of kilobits from clients/#total workers":
        return "# $10^3$ bits/n"
    ...
    if axis == "special_diversity_not_scaled":
        return "Special diversity not scaled"

    if axis == "delta_c1_c2":
        return "$\\langle \\Delta_{c1}, \\Delta_{c2} \\rangle$"

    if axis == "delta_ci_cj_avg":
        return "$avg\\left(\\langle \\Delta_{ci}, \\Delta_{cj} \\rangle\\right)$"

    if axis == "Sigma":
        return "$\\sigma$"

    if axis == "Sigma from Z'":
        return "$\\sigma$ based on Z'"
    ...
    if axis == "L_plus_one":
        return "$L_+$ (from Assumption 4)"

    if axis == "L_plus_two":
        return "$L_+$ (from Lemma 2)"

    if axis == "sqrt_of_avg_Li_sqr":
        return "$\\sqrt{\\frac{1}{n}\\sum_{i} (L_i^2) }$"

    if axis == "sqrt_of_max_Li_sqr":
        return "$\\sqrt{\\frac{c}{n}\\max_{i} (L_i^2) }$"

    if axis == "gamma":
        return "$\\gamma$"

    if axis == "Running function value (proxy train)":
        return "$\\hat{f}(x)$"

    if axis == "Norm of server stochastic gradient (proxy train)":
        return "${||\\nabla \\hat{f}(x)||}$"
...
def getLastFieldInfo(dataset, filepaths, fieldName):
    fname = filepaths[dataset]
    ...
    with open(fname, "rb") as f:
        ...
        for (job_id, H) in obj:
            ...
            last_round = sorted(list(H["history"].keys()))[-1]
            return H["history"][last_round][fieldName]
...
def onPlotResults(export_legend_to_file):
    ...
    for g, job_id in enumerate(job_ids):
        ...
        # Reduce number of rounds by one (to track various quantities)
        rounds -= 1
        ...
        for round in range(rounds):
            ...
            for axis_type, axis_values in ( (xAxisType, X), (yAxisType, Y) ):
                ...
                if axis_type == "special_diversity_not_scaled":
                    axis_values[round] = round_info['special_diversity_not_scaled']
                    Y_sigma[round] = 0.0

                if axis_type == "delta_c1_c2":
                    axis_values[round] = round_info['delta_c1_c2']
                    Y_sigma[round] = 0.0

                if axis_type == "delta_ci_cj_avg":
                    axis_values[round] = round_info['delta_ci_cj_avg']
                    Y_sigma[round] = 0.0

                #======================================================================================================
                if axis_type == "C":
                    if 'C' in round_info:
                        axis_values[round] = round_info['C']
                    Y_sigma[round] = 0.0

                if axis_type == "G":
                    axis_values[round] = round_info['G']
                    Y_sigma[round] = 0.0

                if axis_type == "L_plus_one":
                    axis_values[round] = round_info['L_plus_one']
                    Y_sigma[round] = 0.0

                if axis_type == "L_plus_two":
                    axis_values[round] = round_info['L_plus_two']
                    Y_sigma[round] = 0.0

                if axis_type == "sqrt_of_avg_Li_sqr":
                    axis_values[round] = round_info['sqrt_of_avg_Li_sqr']
                    Y_sigma[round] = 0.0

                if axis_type == "sqrt_of_max_Li_sqr":
                    axis_values[round] = round_info['sqrt_of_max_Li_sqr']
                    Y_sigma[round] = 0.0

                if axis_type == "gamma":

                    if 'gamma' in round_info:
                        axis_values[round] = round_info['gamma']
                    else:
                        axis_values[round] = H['th_stepsize_cvx']

                    Y_sigma[round] = 0.0

                if axis_type == "Sigma":
                    axis_values[round] = round_info['sigma_max_card']
                    Y_sigma[round] = 0.0

                if axis_type == "Sigma from Z'":
                    axis_values[round] = round_info['sigma_max_card_from_data']
                    Y_sigma[round] = 0.0

                if axis_type == "Special Diversity":
                    axis_values[round] = round_info['special_diversity']
                    Y_sigma[round] = 0.0

                if axis_type == "Cos of gradients c1/c2":
                    axis_values[round] = round_info['cos_c1_c2']
                    Y_sigma[round] = round_info['cos_c_sigma']

                if axis_type == "Cos of gradients (ci,cj) - average":
                    axis_values[round] = round_info['cos_c_avg']
                    Y_sigma[round] = round_info['cos_c_sigma']

                if axis_type == "Cos of gradients (ci,cj) - sigma":
                    axis_values[round] = round_info['cos_c_sigma']
                    Y_sigma[round] = 0.0

                if axis_type == "Rounds":
                    axis_values[round] = round

                if axis_type == "Rounds (thousands)":
                    axis_values[round] = round/1000.0
                ...
                if axis_type == "# of kilobits from clients/#total workers":
                    axis_values[round] = (send_scalars_to_master[round] / H["total_clients"] * one_scalar_in_bits) / 1000.0
        ...
        ds_with_grad_x0 = {"phishing" : grad_zeros_path + "phishing_start_grad_sqr.bin",
                           "mushrooms": grad_zeros_path +  "mushrooms_start_grad_sqr.bin",
                           "a9a": grad_zeros_path +  "a9a_start_grad_sqr.bin",
                           "w5a": grad_zeros_path + "w5a_start_grad_sqr.bin",
                          }

        ds_with_grad_x0_for_non_cvx =  {
            "c_div_n:0.05": grad_zeros_path + "c_div_n_0_05_grad_zero_noncvx.bin",
            "c_div_n:0.5": grad_zeros_path + "c_div_n_0_5_grad_zero_noncvx.bin",
            "c_div_n:0.9": grad_zeros_path + "c_div_n_0_9_grad_zero_noncvx.bin"
        }

        ds_with_grad_x0_for_cvx =  {
            "c_div_n:0.05": grad_zeros_path + "c_div_n_0_05_grad_zero_cvx.bin",
            "c_div_n:0.5": grad_zeros_path + "c_div_n_0_5_grad_zero_cvx.bin",
            "c_div_n:0.9": grad_zeros_path + "c_div_n_0_9_grad_zero_cvx.bin"
        }

        if H["args"].global_regulizer == 'noncvx_robust_linear_regression':
            used_dataset_map_for_syntetics = ds_with_grad_x0_for_non_cvx
        else:
            used_dataset_map_for_syntetics = ds_with_grad_x0_for_cvx

        # =================================================================================================================
        if H["args"].dataset in ds_with_grad_x0:
            if yAxisType == "Norm of function gradient square(train)":
                grad_at_xo = getLastFieldInfo(H["args"].dataset, ds_with_grad_x0  , "full_gradient_norm_train")
                Y = [grad_at_xo * grad_at_xo] + Y
            elif yAxisType == "Norm of function gradient (train)":
                grad_at_xo = getLastFieldInfo(H["args"].dataset, ds_with_grad_x0, "full_gradient_norm_train")
                Y = [grad_at_xo] + Y

        elif H["args"].dataset.lower() == "generated_for_quadratic_minimization":
            options = H["args"].algorithm_options.split(",")
            mode = None

            if "c_div_n:0.05" in options:
                mode = "c_div_n:0.05"
            elif "c_div_n:0.5" in options:
                mode = "c_div_n:0.5"
            elif "c_div_n:0.9" in options:
                mode = "c_div_n:0.9"

            if mode != None:
                if yAxisType == "Norm of function gradient square(train)":
                    grad_at_xo = getLastFieldInfo(mode, used_dataset_map_for_syntetics, "full_gradient_norm_train")
                    Y = [grad_at_xo * grad_at_xo] + Y
                elif yAxisType == "Norm of function gradient (train)":
                    grad_at_xo = getLastFieldInfo(mode, used_dataset_map_for_syntetics, "full_gradient_norm_train")
                    Y = [grad_at_xo] + Y
        ...
        # =================================================================================================================
        #markers_numbers = [22, 10, 3000, 3000, 13, 15, 16, 17, 19]
        markers_numbers = [22, 10, 15, 15, 13, 15, 16, 17, 19]
        markers_count_selection = markers_numbers[g % len(markers_numbers)]      # markers_numbers[gui_rnd.randint(0, len(markers_numbers)-1)]
    ...
    if analysisView.cbxLegendLogScalingY.checkState() == Qt.Checked and doNotAggregate:
        for p in plots:
            ...
            if plot_error_bar:
                ax.fill_between(p["X"], [p["Y"][i]  - Y_sigma[i] for i in range(len(p["Y"]))], [p["Y"][i] + Y_sigma[i] for i in range(len(p["Y"]))], alpha=0.2)
